...
class HospitalApp(tk.Tk):
    ...
    def show_register(self):
        ...
        def do_reg():
            ...
            safe_query("INSERT INTO USER_ACCOUNT (user_id,username,password_hash,email,role,profile_id,profile_type) VALUES (%s,%s,%s,%s,'Patient',%s,'PATIENT')", (v['pid'], v['user'], v['pass'], v['email'], v['pid']), fetch=False)
            safe_query("INSERT INTO PATIENT (patient_number,ssn,name) VALUES (%s,%s,%s)", (v['pid'], v['ssn'], v['name']), fetch=False); messagebox.showinfo("Done", "Account Created!"); self.show_login()
